...
- ドメインデータ（items, attachments 等）：SQLite
- Settings（言語・テーマ等の非機密）：AsyncStorage
- ProState（課金状態等の機密）：SecureStore（OS の暗号化ストレージ）
- 写真 / 音声等のファイル実体：アプリのドキュメント領域（パスは **必ず相対パス**で DB 保存。詳細は `src/db/filePathUtils.ts`）
...
- **Free にバナー広告（AdMob）を組み込む**（Pro は常に広告ゼロ）
- 開発中は **テスト広告ユニット** を使う（本番広告を開発で表示しない → AdMob アカウント BAN リスク）
- 本番の広告ユニット ID は **ソースに直書きしない**（`.env` から `app.config.ts` 経由で注入）

---

### 2-4. i18n（言語 / レイアウト）の方針

- v1.x は **19 言語対応**（en, ja, fr, es, de, it, pt, ru, zh-Hans, zh-Hant, ko, hi, id, th, vi, tr, nl, pl, sv）
- アラビア語は除外（RTL 対応しないため）
- レイアウトは **LTR のみ**（RTL はサポート外）
- 言語コードは **BCP47 準拠** を前提とし、**オランダ語は `nl` を正**とする
