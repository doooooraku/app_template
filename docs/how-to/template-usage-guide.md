...
## Phase 1.5: Claude Code Skills のインストール（最初の 1 回のみ）
...
現行の Claude Code では不可能ですが、将来対応された場合に備えて、このテンプレートでは `.claude/skills/<name>/SKILL.md` の構造で同梱しています。
...
| **EAS プロジェクトID**     | EAS のプロジェクト識別子           | `xxxxxxxx-xxxx-...`          | 空欄で自動作成（推奨）   |

### EAS プロジェクト ID について

EAS プロジェクト ID は、Expo のクラウドビルドやOTAアップデートに必要です。

**空欄にした場合（推奨）:**
setup.sh が自動的に EAS プロジェクトを作成し、ID を `.env` に書き込みます。事前に `eas login` でログインしておいてください。

**手動で入力する場合:**
既存の EAS プロジェクトがある場合は、UUID を直接入力できます。

> **💡 EAS 未ログインの場合:** setup.sh は EAS 初期化をスキップします。後から `eas login` → `eas init` → `.env` の `EAS_PROJECT_ID=` に値を入れれば OK です。
...
6. **EAS プロジェクトを自動作成**（ログイン済みの場合）
7. `pnpm verify` で全チェックを実行
